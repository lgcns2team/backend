...
-- ##################################################
-- 주요사건 데이터
INSERT INTO main_event (event_id, country_id, event_name, year, era, description, summary) VALUES
    -- 통일 신라 말
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '통일 신라'), 
      '혜공왕 피살 및 왕위 다툼 심화', 780, '신라 말', 
      '진골 귀족들이 반란을 일으켜 혜공왕이 살해당하고 무열왕계 왕위 세습이 단절된다. 이 사건 이후 중앙 귀족 간의 왕위 쟁탈전이 극심해지며 정치가 매우 불안정해진다. 약 150여 년 동안 무려 20여 명의 왕이 교체될 정도로 왕권이 약화된다. 이는 신라 하대 사회 혼란을 가속화하는 중요한 계기가 된다.',
      '진골 귀족들의 반란으로 혜공왕이 살해당하고 무열왕계 왕위 세습이 단절됨.'
    ),
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '통일 신라'), 
      '장보고, 청해진 설치', 828, '신라 말', 
      '당나라에서 활동하던 장보고가 흥덕왕에게 건의하여 완도에 해상 거점인 청해진을 설치한다. 그는 청해진을 근거지로 하여 신라, 당, 일본 간의 해상 무역로를 장악한다. 장보고는 해적들을 소탕하여 동아시아 해상 무역의 안전을 확보하고 무역을 독점한다. 이를 통해 청해진은 동아시아 삼국을 잇는 중계 무역의 중심지로 성장하며 장보고는 해상왕으로 불린다.',
      '장보고가 완도에 청해진을 설치하고 해적들을 소탕하며 해상 무역을 주도함.'
    ),
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '통일 신라'), 
      '김헌창의 난', 822, '신라 말', 
      '웅주 도독이었던 김헌창이 자신의 아버지가 왕이 되지 못한 것에 불만을 품고 반란을 일으킨다. 그는 국호를 장안이라 칭하고 스스로 왕이라 선포하며 지방 군사력을 동원한다. 이 반란은 지방 세력이 중앙 정부의 통제에서 벗어나 반기를 든 대표적인 사건이다. 비록 진압되었으나, 이는 이미 신라 중앙 정부의 지방 통제력이 크게 약화되었음을 명확히 보여준다.',
      '웅주 도독 김헌창이 국호를 장안이라 칭하며 반란을 일으켰으나 진압됨.'
    ),
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '통일 신라'), 
      '원종·애노의 난', 889, '신라 말', 
      '진성 여왕 시기에 중앙 정부가 재정 확보를 위해 지방에 가혹하게 세금을 독촉하자 이에 반발하여 일어난 농민 봉기이다. 사벌주(상주)에서 원종과 애노가 주도하여 시작된 이 봉기는 전국으로 급속하게 확대된다. 이 사건을 기점으로 농민 봉기가 전국에서 빈발하며 신라 사회의 혼란이 심화된다. 중앙 정부가 이 봉기들을 효과적으로 진압하지 못하면서 지방에 대한 통제력을 완전히 상실하는 계기가 된다.',
      '중앙 정부의 가혹한 세금 독촉에 반발하여 사벌주에서 농민 봉기가 시작되어 전국으로 확산됨.'
    ),

    -- 후삼국 시대 성립
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '후백제'), 
      '견훤, 후백제 건국', 900, '후삼국 시대', 
      '군인 출신이었던 견훤이 무진주(광주)를 점령하고 세력을 키운 후, 완산주(전주)를 도읍으로 정하고 후백제를 건국한다. 그는 고구려가 아닌 백제의 부흥을 명분으로 내세우며 왕위에 오른다. 이를 통해 신라 말의 분열된 정세 속에서 후삼국 시대가 본격적으로 시작된다. 견훤은 일부 6두품 세력을 등용하여 통치 체제를 정비하고 신라를 압박한다.',
      '서남 해안 군인 출신 견훤이 완산주를 도읍으로 후백제를 건국함.'
    ),
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '후고구려(태봉)'), 
      '궁예, 후고구려 건국', 901, '후삼국 시대', 
      '신라 왕실 출신으로 알려진 궁예가 송악(개성)을 근거지로 삼아 후고구려를 세운다. 그는 이후 국호를 마진으로 바꾸고 철원으로 천도하며, 다시 국호를 태봉으로 변경한다. 궁예는 호족들의 군사적 도움을 받아 왕국을 성립시킨다. 이후 최고 관청인 광평성을 설치하고 독자적인 통치 제도를 정비하며 왕권을 강화한다.',
      '궁예가 송악을 근거지로 후고구려를 세우고 국호를 마진과 태봉으로 변경하고 철원으로 천도함.'
    ),
    
    -- 고려 초기
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '고려'), 
      '왕건, 고려 건국 및 송악 천도', 918, '후삼국 시대', 
      '궁예의 폭정이 심해지자 송악 지역의 호족들과 왕건을 따르는 세력이 궁예를 몰아내고 왕건을 새로운 왕으로 추대한다. 왕건은 국호를 고려라 정하고 고구려의 계승 의식을 천명한다. 이듬해인 919년에는 철원에서 자신의 기반 지역인 송악(개성)으로 수도를 옮긴다. 송악은 방어에 유리하고 예성강을 통한 대외 교역(벽란도)에도 유리한 지리적 이점을 가진 곳이었다.',
      '궁예를 몰아내고 왕건이 고려를 건국하고 이듬해 송악으로 천도함.'
    ),
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '고려'), 
      '신라 경순왕, 고려에 항복', 935, '후삼국 통일', 
      '신라의 마지막 왕인 경순왕이 더 이상 나라를 유지하기 어렵다고 판단하고 고려에 평화적으로 나라를 넘겨주기로 결정한다. 경순왕은 당시 고려의 왕이었던 왕건에게 신라를 통째로 귀순시킨다. 왕건은 경순왕을 극진히 대우하고 신라의 유민과 귀족들을 포용하는 정책을 편다. 이는 후삼국을 무력 통일하기 이전에 신라 세력을 흡수하여 민족 재통합의 기반을 마련한 중요한 사건이다.',
      '신라의 마지막 왕 경순왕이 나라를 유지하기 어렵다고 여겨 고려에 스스로 항복함.'
    ),
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '고려'), 
      '고려, 후삼국 통일 완성', 936, '후삼국 통일', 
      '신라의 항복 이후, 왕건은 견훤의 귀순을 받아들이고 후백제와의 마지막 결전인 일리천 전투에서 대승을 거둔다. 이 승리로 고려는 후백제를 완전히 격파하고 마침내 후삼국 통일을 완성한다. 고려는 신라와 후백제 유민뿐만 아니라 발해 유민까지 대거 포용한다. 이는 민족의 재통합을 이루어 현재의 한반도 국가의 기틀을 확립한 역사적인 사건이다.',
      '견훤의 귀순과 후백제 격파를 통해 고려가 후삼국을 통일 완성함.'
    ),
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '고려'), 
      '광종, 노비안검법 실시', 956, '고려 초기', 
      '광종이 즉위 후 왕권을 강화하기 위해 실시한 일련의 개혁 조치 중 하나이다. 이 법은 권세 있는 호족이나 공신들이 불법적으로 노비로 삼은 사람들을 조사하여 양인 신분으로 되돌려주는 것을 목적으로 한다. 노비안검법의 시행은 호족 세력의 경제적 기반인 토지와 군사적 기반인 사병을 약화시킨다. 동시에 양인이 증가하면서 국가의 세금과 군역 대상이 늘어나 국가 재정이 확충된다.',
      '광종이 권세가에게 불법적으로 노비가 된 사람들을 조사하여 양인 신분으로 되돌려준 법을 실시함.'
    ),
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '고려'), 
      '광종, 과거제 실시', 958, '고려 초기', 
      '광종은 중국에서 온 쌍기의 건의를 받아들여 유교적 소양을 갖춘 인재를 등용하기 위한 과거제를 시행한다. 이 제도는 혈통보다는 능력과 학식을 바탕으로 관료를 선발하는 것을 목표로 한다. 과거제를 통해 신진 관료를 등용함으로써 기존 호족 세력의 힘을 견제한다. 이는 국왕 중심의 관료제 통치 질서를 확립하고 왕권 강화를 이루는 데 결정적으로 기여한다.',
      '광종이 쌍기의 건의로 유교적 소양을 갖춘 인재를 등용하기 위해 과거제를 시행함.'
    ),
    
    -- 고려 전기
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '고려'), 
      '서희, 외교 담판 및 강동 6주 확보', 993, '고려 전기', 
      '거란이 1차 침입을 감행했을 때, 고려의 외교관이었던 서희가 거란의 장수 소손녕과 단독으로 담판을 벌인다. 서희는 고려가 고구려를 계승한 국가임을 내세우며 거란이 공격하는 이유를 무마시킨다. 서희는 송과의 외교 단절을 약속하는 대신 여진족이 차지하고 있던 압록강 하류 지역의 영토를 확보한다. 이를 통해 고려는 군사적 충돌 없이 오히려 압록강 동쪽 지역의 강동 6주를 확보하는 외교적 승리를 거둔다.',
      '거란의 1차 침입 시 서희가 외교 담판을 벌여 강동 6주를 확보함.'
    ),
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '고려'), 
      '강감찬, 귀주대첩 승리', 1019, '고려 전기', 
      '거란이 고려를 세 번째로 대규모 침입했을 때, 강감찬이 총지휘관이 되어 고려군을 이끈다. 강감찬은 흥화진에서 거란군을 미리 매복하여 크게 타격을 입힌다. 이후 후퇴하는 거란군을 귀주에서 최종적으로 포위하여 전멸에 가까운 대승을 거둔다. 이 귀주대첩 승리는 거란의 침략 의지를 완전히 꺾고 송-요-고려의 동아시아 다원적 국제 질서를 유지하는 데 결정적인 기여를 한다.',
      '거란의 3차 침입 시 강감찬이 지휘한 고려군이 귀주에서 거란군을 크게 물리침.'
    ),
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '고려'), 
      '윤관, 동북 9성 축조', 1108, '고려 전기', 
      '여진족이 고려 국경을 침범하며 세력을 키우자, 윤관은 여진족과의 전투에 특화된 기병 중심의 특수 부대인 별무반을 조직한다. 윤관은 별무반을 이끌고 여진을 대대적으로 정벌한 후 동북 지역에 9개의 성을 쌓는다. 이 동북 9성은 고려의 북방 영토 확장을 보여주는 사건이다. 그러나 험준한 지형과 여진의 끊임없는 공격, 그리고 방어의 어려움으로 인해 고려는 결국 여진의 요구를 받고 1년여 만에 9성을 반환한다.',
      '윤관이 별무반을 이끌고 여진을 정벌한 후 동북 9성을 쌓았으나 1년여 만에 반환함.'
    ),
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '고려'), 
      '이자겸의 난', 1126, '고려 전기', 
      '경원 이씨의 최고 권력자였던 이자겸이 자신의 외손자인 인종을 허수아비로 만들고 권력을 독점하려 한다. 그는 심지어 왕궁에 불을 지르고 인종을 위협하는 등 전횡을 일삼는다. 인종이 김찬과 안보린을 시켜 이자겸을 제거하려 했으나 실패하고 이자겸이 오히려 권력을 장악한다. 이는 고려 문벌 귀족 사회의 내부 모순과 외척 세력의 권력 독점을 단적으로 보여주었으며, 문벌 사회의 동요를 가속화한다.',
      '경원 이씨 이자겸이 척준경과 함께 난을 일으켜 왕권을 위협함.'
    ),
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '고려'), 
      '묘청의 서경 천도 운동', 1135, '고려 전기', 
      '풍수지리설을 내세운 묘청 등 서경파가 개경의 문벌 귀족 세력을 견제하기 위해 서경(평양)으로의 천도를 주장한다. 그들은 또한 금나라를 정벌하여 자주적인 국가를 건설해야 한다고 주장한다. 그러나 보수적인 문벌 귀족 대표인 김부식을 중심으로 한 개경파의 반대에 부딪힌다. 묘청은 결국 반란을 일으켰으나 김부식이 지휘하는 관군에 의해 진압되어 자주적이고 개혁적인 서경파가 보수적인 개경파에게 패배한다.',
      '묘청 등이 풍수지리설을 내세워 서경 천도와 금 정벌을 주장했으나 김부식의 관군에 의해 진압됨.'
    ),
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '고려'), 
      '무신정변 발생', 1170, '무신 정권', 
      '문신 중심의 사회에서 무신들이 오랫동안 받아온 차별과 무시, 그리고 의종의 문신 우대 정책에 대한 불만이 폭발한다. 정중부, 이의방 등의 무신들이 정변을 일으켜 문신들을 살해하고 권력을 장악한다. 이 정변으로 문신 중심의 문벌 귀족 사회가 몰락하고 약 100년간 무신이 정치를 주도하는 무신 정권 시대를 연다. 무신들은 최고 회의 기구인 중방(후에 교정도감 등)을 중심으로 국정을 처리한다.',
      '문신 중심 사회의 무신 차별에 불만을 품은 무신들이 정변을 일으켜 권력을 장악함.'
    ),

    -- 무신 정권기 하층민 봉기
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '고려'), 
      '망이·망소이의 난', 1176, '무신 정권', 
      '무신 정권의 가혹한 수탈과 지방에 대한 차별에 반발하여 공주 명학소 주민들이 일으킨 대규모 농민 봉기이다. 명학소는 일반 군현보다 차별받던 특수 행정 구역으로, 주민들의 불만이 더욱 컸다. 이들은 잠시 공주를 점령하는 등 무신 정권에 큰 위협을 가한다. 이 난은 하층민들이 신분적 차별과 지배층의 수탈에 대해 적극적으로 저항하는 의식을 보여준 대표적인 사건이다.',
      '공주 명학소 주민들이 무신 정권의 수탈에 반발하여 일으킨 봉기임.'
    ),
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '고려'), 
      '만적의 봉기 계획', 1198, '무신 정권', 
      '당시 최고 권력자였던 최충헌의 사노비였던 만적이 주도하여 노비 해방과 정권 탈취를 계획한 사건이다. 만적은 "장군이나 재상에 어찌 씨가 따로 있겠느냐"며 천민들의 신분적 차별 극복 의지를 고취시킨다. 그는 노비들을 모아 대규모 봉기를 시도하려 했으나, 사전에 계획이 누설되어 주모자들이 체포되면서 실패한다. 이는 무신 정권 하에서 노비와 천민 계층의 신분 해방 의식이 성장했음을 보여준다.',
      '최충헌의 사노비 만적이 노비 해방과 정권 탈취를 계획했으나 사전에 발각되어 실패함.'
    ),

    -- 대몽 항쟁 및 원 간섭기
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '고려'), 
      '고려, 강화도 천도', 1232, '대몽 항쟁', 
      '몽골이 재침입하여 개경이 위험에 처하자, 당시 최고 권력자였던 최우가 수도를 강화도로 옮긴다. 최우는 육상전에 강한 몽골군에 비해 바다를 끼고 있는 강화도가 방어에 유리하다고 판단한다. 천도를 통해 고려 조정과 무신 정권은 몽골에 대항하는 장기적인 항전의 발판을 마련한다. 하지만 이로 인해 개경에 남아 있던 백성들은 몽골군의 공격에 무방비로 노출되는 결과를 낳는다.',
      '몽골의 재침입에 대비하여 당시 권력자 최우가 방어에 유리한 강화도로 수도를 옮김.'
    ),
    (gen_random_uuid(), (SELECT country_id FROM country WHERE country_name = '고려'), 
      '몽골과 강화 체결 및 개경 환도', 1270, '원 간섭기', 
      '무신 정권이 붕괴된 후, 고려 정부는 몽골과의 전쟁을 끝내기 위해 강화를 맺기로 결정한다. 강화 체결 후, 몽골의 압력과 요구에 따라 고려 조정은 강화도를 떠나 다시 원래의 수도인 개경으로 돌아온다. 수십 년간 지속된 대몽 항쟁은 이로써 공식적으로 종결된다. 개경 환도 이후 고려는 원나라의 내정 간섭을 받는 원 간섭기가 시작되며, 이에 반발한 삼별초는 항쟁을 이어간다.',
      '고려 정부가 몽골과 강화를 맺고 강화도에서 개경으로 돌아와 원 간섭기가 시작됨.'
    );
